...
features_DF = pred_data.toPandas()
#print(features_DF)
...
first, second = split(features_DF, 1)


fistDF=spark.createDataFrame(first) 
secondDF=spark.createDataFrame(second)


features = assembler.transform(fistDF).select('features')
indexPredicted1 = int(model.predict(features.first()['features']))
print(user_lab[indexPredicted1])

features = assembler.transform(secondDF).select('features')
indexPredicted2 = int(model.predict(features.first()['features']))
print(user_lab[indexPredicted2])

final_32 = open("/Users/juancasarescosmen/out/out_3_2.txt", "w")
final_32.write("Class" + "\n")
final_32.write(user_lab[indexPredicted1])
final_32.write("\n")
final_32.write(user_lab[indexPredicted2])
final_32.close()
#indexPredicted1 = int(model.predict(features.first()['features']))
#print(indexPredicted1)
